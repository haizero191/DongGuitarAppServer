...
class OrderController {
  async index(req, res) {
    ...
    try {
      var Get_Order_Result = await Order.find(query)
        .sort("-UpdateAt")
        .populate({
          path: "Product",
          select: "Name",
  ...
  async create(req, res) {
    ...
    const product = await Product.find({ _id: req.body.Product })
      .populate("Brand")
      .populate("Category");
    if (product[0]) {
      ...
      if (Create_Order_Result) {
        ...
        res.json(
          responseJSON("Order created successfully!", Create_Order_Result, true)
        );
      } else {
  ...
  async count(req, res) {
    ...
    try {
      var Count_Order_Result = await Order.countDocuments(query);
      res.json(
        responseJSON("Get count order success", Count_Order_Result, true)
      );
    } catch (error) {
      res.json(responseJSON("Get count order failed", 0, false));
    }
  }

  async search(req, res) {
    var { code, phone } = req.query;
    var query = {};
    ...
    try {
      ...
      res.json(responseJSON("Find Order success !", Search_Order_Result, true));
    } catch (error) {
      res.json(responseJSON("Find Order success !", [], false));
    }
  }
      ...
      if (Object.keys(dataUpdate).length === 0) {
        ...
        Order.findByIdAndUpdate(id, dataUpdate, { new: false })
          .then((updateOrder) => {
            ...
            res.json(
              responseJSON("Order update successfully", updateOrder, true)
            );
          })
          .catch((error) => {
